...
// URL constants
const API_BASE_URL = "http://192.168.21.193:5000";
...
function App() {
  ...
  const fetchDrivers = async () => {
    try {
      ...
      // Cập nhật marker hiện có hoặc thêm mới
  ...
  return (
    <div className="App">
      <header className="App-header">
        <h1>Ứng dụng Đặt Xe</h1>
        <div className="connection-status">
          {socket.current?.connected ? (
            <span className="connected">● Đã kết nối</span>
          ) : (
            <span className="disconnected">● Mất kết nối</span>
          )}
        </div>
      </header>
      ...
      <div className="content">
        ...
        <div className="booking-panel">
          ...
          <div className="driver-list">
            <h3>Tài xế đang hoạt động ({drivers.length})</h3>
            <ul>
              {drivers.map((driver) => (
                <li key={driver.id}>
